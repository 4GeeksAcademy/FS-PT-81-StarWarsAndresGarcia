...
export const SingularStarship = () => {
    const params = useParams()
    const { store, actions } = useContext(Context);
    useEffect(() => {
        actions.getStarship(params.id)
    }, [params.uid])
    return (
        <div className="container d-flex bg-dark">
            <div>
                <h1 className="titulo text-light">{store.singular?.name}</h1>
                <img src={`https://starwars-visualguide.com/assets/img/starships/${params.id}.jpg`} alt="..." />
            </div>
            <div className="d-flex justify-content-end fit-contain flex-column">
                ...
                <div className="d-flex justify-content-between gap-3 mt-3 text-light">
                    ...
                    <div className="info-box">
                        <h5>Consumables</h5>
                        <hr></hr>
                        <p>{store.singular?.consumables}</p>
                    </div>
                    <div className="info-box">
                        <h5>Created</h5>
                        <hr></hr>
                        <p>{store.singular?.created}</p>
                    </div>
                    <div className="info-box">
                        <h5>Crew</h5>
                        <hr></hr>
                        <p>{store.singular?.crew}</p>
                    </div>
                    <div className="info-box">
                        <h5>Edited</h5>
                        <hr></hr>
                        <p>{store.singular?.edited}</p>
                    </div>
                </div>
            </div>
        </div>
